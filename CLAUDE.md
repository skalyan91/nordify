...
**`_edge_blur(image, sigma, edges)`** — builds `_N_SCALE_LEVELS` (6) blur levels with sigma values at `σ_max·(i/N)^(p/(2p−1))` — the quantile distribution whose density is proportional to `|f'(mask)|` for the mapping `f(mask) = mask^p` (`_BLUR_MAP_EXP=2`, quadratic). This places more levels near `σ_max` where the mapping changes fastest, minimising interpolation error. Mask→level mapping: `level = mask^p · N`. Scale-space interpolation avoids ghosting. All operations in linear light. Controlled via `--blur PCT` and `--edges EDGES`.
