...
**`_edge_blur(image, sigma, edges)`** — builds `_N_SCALE_LEVELS` (6) blur levels at σ/6, 2σ/6, …, σ and interpolates across them. The mask-to-level mapping uses `mask^_BLUR_EXPONENT` (exponent=2), so sigma grows quadratically with distance from the edge — very gradual onset near the centre, steeper near the edge. Scale-space interpolation (rather than sharp/blurred blend) avoids ghosting. All operations in linear light. Controlled via `--blur PCT` and `--edges EDGES`.
