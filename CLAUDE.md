...
**`_edge_blur(image, sigma, edges)`** — builds `_N_SCALE_LEVELS` (6) blur levels at σ/6, 2σ/6, …, σ and interpolates across them. The mask-to-level mapping uses the normalised exponential `(exp(k·mask)−1)/(exp(k)−1)` with `_BLUR_CURVE_K=5`, keeping sigma near-zero across most of the ramp and rising steeply only near the edge. Scale-space interpolation (rather than sharp/blurred blend) avoids ghosting. All operations in linear light. Controlled via `--blur PCT` and `--edges EDGES`.
