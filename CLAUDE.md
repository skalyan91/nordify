...
**`_nighttime(image, light_boost=0.2)`** — darkens and cools the image in Oklab before palette conversion: `L → 1 − √(1 − L)` (a curve that darkens mid-tones and highlights while keeping shadows from crushing to black) and `b` shifted toward blue in inverse proportion to `L` (dark pixels get the full shift, bright pixels are left unchanged, so highlights keep their original colour temperature). Both formulas are unchanged from the original implementation — an intermediate version tried protecting vivid-but-dim pixels (e.g. a lit window that isn't literally bright) by blending brightness protection with a chroma-derived signal, but any formulation that lets chroma influence the *same* protection signal as lightness ends up double-counting brightness protection for pixels where lightness alone is already high (most of an ordinary bright, sunlit photo), broadly under-darkening the whole image, not just genuinely vivid content — confirmed: mean lightness after the transform dropped only half as much on a bright test photo under such a design as under the original formula. Reverted in favour of a structurally separate mechanism (below) that leaves the base formulas untouched.
