...
**`_edge_blur(image, sigma, ramp_px, edges)`** — builds `_N_SCALE_LEVELS` (50) evenly-spaced blur levels (`σᵢ = σ_max·i/N`). Mask→level mapping: `level = mask^p·N` (`_BLUR_MASK_POWER=8`). `sigma` (max blur, default 2% of height) and `ramp_px` (spatial ramp extent, default 50% of height) are independent parameters. The 8th-power mapping keeps the centre sharp and concentrates blur tightly at the edge. Scale-space interpolation avoids ghosting. All operations in linear light. Controlled via `--blur PCT`, `--ramp PCT`, and `--edges EDGES`.
