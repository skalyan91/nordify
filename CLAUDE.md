...
**`_detect_figure_focus(depth_raw, image_bgr, sam_model="facebook/sam-vit-base", target_width=1500, min_area_frac=0.0005, max_area_frac=0.5)`** — candidates come from SAM's automatic mask generation on the *source image*, the same trade `_flatten_thin_masts` already makes and for the same reason. An earlier version ran classical Canny edge detection directly on the depth map and connected-component-labelled the non-edge pixels — exactly what `_flatten_thin_masts` originally did before switching to SAM. It worked on photos with sharp depth discontinuities but failed on a stylised power-station painting: Canny traced the cooling tower's silhouette as a visually clear gradient, but the boundary had gaps too small to see by eye and too large for a fixed-size dilation to close at that image's resolution, so the tower's "inside" leaked into the surrounding sky and merged into one ~90%-of-frame connected blob (confirmed by rendering that raw component). The tower was never even considered as a candidate — no amount of downstream ranking can recover from a segmentation stage that never isolated it. SAM has no such failure mode: it segments from the image's own visual boundaries, complete and unambiguous even where the depth map's inferred discontinuity is not.

The image is downsampled to `target_width` before running SAM, exactly as `_flatten_thin_masts` does (same ~60x cost/no-quality-loss trade). Each mask is resized back to `depth_raw`'s resolution with nearest-neighbour interpolation before anything measures it.

A single object can still come back as several SAM masks — SAM's output is hierarchical (whole object and sub-parts both proposed), and an occluder can split one mask into two — even though every piece sits at the same depth. Regions whose depth ranges (10th–90th percentile of their own pixels) overlap by more than 50% of *both* ranges' own extents (**`_ranges_overlap_50`**) are merged via union-find (**`_UnionFind`**) into one candidate — a graph problem, not a single pairwise pass, since three regions can chain together (A–B and B–C both overlap enough) even if A and C don't directly overlap.

Any region touching the image border is dropped **before** merging, not after: a border-touching region is a partial view of whatever it belongs to (cropped by the photo, not by a real object boundary), so nothing measured from it — size, shape, depth — can be trusted. This has to happen pre-merge because merging runs on depth-range overlap alone, blind to spatial position: a small, unrelated, border-touching mask (a sliver of sky) can coincidentally share a large interior candidate's depth range, and if the border check ran post-merge it would disqualify the *whole* merged group on that coincidence — confirmed: a 5,669px border-touching mask merged into, and discarded, an otherwise-clean 348,847px cooling-tower group before this was fixed. Filtering first sidesteps it entirely: a union of masks that each avoid the border can never touch the border either.

Each merged candidate is scored on five criteria and combined by a **weighted rank sum** (**`_rank`**, which averages tied ranks rather than breaking ties by list order — important because `connectedness` genuinely ties at 1.0 for most candidates, a single coherent SAM mask, and an unbroken tie shouldn't swing a double-weighted criterion on nothing but incidental list order; confirmed: two candidates tied at connectedness=1.0 landed at ranks 2 and 12 out of 14 before this was fixed):

- **area** — pixel count. Weight 6, the heaviest — confirmed necessary when a correctly-segmented 257,310px cooling tower lost outright to an incidental 23,824px dark-bush blob that was merely shaped slightly better (higher chunkiness, solidity, nearer depth), before area's weight was raised from 1. 6 is enough to overcome losing on any two of the double-weighted shape criteria (6 > 2+2) but not enough to overcome all three plus depth (6 ≤ 2+2+2).
- **chunkiness** (**`_chunkiness(mask, area, erosion_frac=0.1)`**) — fraction of the mask's area surviving erosion by a disk of radius `0.1 × sqrt(area/π)` (its own equivalent-circle radius). Weight 2. Replaces bounding-box aspect ratio ("squareness") as the elongation-tolerant compactness criterion: aspect ratio can't distinguish a solid, tall structure (a cooling tower or chimney — substantial cross-section at every height) from a thin sliver of the same elongation (a wire, a lattice-tower strut, a noise fragment) — both score near 0 on `min(w,h)/max(w,h)`. Erosion tests the thing that actually differs: local thickness. A thin sliver's width is small in absolute terms, so eroding by even a modest radius wipes it out almost entirely; a solid shape's cross-section survives a proportionally-sized erosion largely intact no matter how tall it is, since a long solid shape's boundary is almost all sides, not ends. Confirmed on synthetic masks: sliver → 0.0, tall solid tower → 0.67, comparably-sized square → 0.79.
- **solidity** — mask area / convex-hull area. Weight 2. A merge that stitched together far-apart same-depth pieces produces a hull that balloons out to cover the gap between them, tanking this score.
- **connectedness** — merged mask's largest single spatial blob / its total area. Weight 2. Depth-range merging is blind to spatial position, so this is what penalises merging two same-depth but spatially disjoint objects — solidity penalises the *gap* between such pieces but not fragmentation with no gap-spanning hull cost.
- **median depth** — higher (nearer) wins. Weight 1.

`min_area_frac` defaults small (0.05% of the image) rather than to a size that already reads as "a subject": a real figure can be genuinely small in frame (a single piece of fruit among many, a distant animal), and rejecting it here would leave the ranking criteria above with nothing legitimate to find. `max_area_frac` (50%) rejects background spanning most of the frame. Falls back to `d_focus=0.0` (background/infinity, the module's pre-existing default) if no candidate survives.
