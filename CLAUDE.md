...
**`_edge_blur(image, sigma, edges)`** — builds `_N_SCALE_LEVELS` (50) blur levels with sigma values at the quantile distribution for density ∝ `exp(k·mask)`: `σᵢ = σ_max·(√(1+expm1(2k)·i/N)−1)/expm1(k)` (`_BLUR_CURVE_K=5`). Mask→level mapping: `level = expm1(k·mask)/expm1(k)·N`. Levels are densest near `σ_max` where the mapping changes fastest. Scale-space interpolation avoids ghosting. All operations in linear light. Controlled via `--blur PCT` and `--edges EDGES`.
